...
public final class Generator {
   ...
   public final Agent[] createAgents(final Station station) {
       ...
       for (int i = n2; i < agents.length; ++i) {
           ...
           if (agents[i].getType() == 6) {
               ...
               agents[i].setMission(this.createFreelanceMission(agents[i]));
           }
       }
   ...
   public final Mission createFreelanceMission(final Agent agent) {
       ...
       int i = this.pickNextDestination(loadSystemsBinary, agent.getStationId());
       if (Status.getSystem().getId() == 15) {
       ...
       int missionType = 0;
       while (j == 0) {
           if ((missionType = GlobalStatus.random.nextInt(13)) != 8) {
               j = 1;
           }
       }
       ...
       if (missionType == 11 || missionType == 0) {
           while (i == Status.getStation().getId()) {
               i = this.pickNextDestination(loadSystemsBinary, agent.getStationId());
           }
       }
       ...
       else if (missionType == 3 || missionType == 5) {
